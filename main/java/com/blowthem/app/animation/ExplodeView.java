package com.blowthem.app.animation;
...
public class ExplodeView extends View{
    ...
    private static class BitmapFragment{
        ...
        public void prepare(int xOrigin, int yOrigin){
            ...
            for(float j = 0; j <= 1; j +=.2){
                tri.lineTo(rnd.nextInt(sliceW), sliceH * j);
            }
            tri.lineTo(0, sliceH);
            tri.close();
    ...
    private void init(Context context) {
        ...
        Bitmap original = BitmapFactory.decodeResource(context.getResources(), R.drawable.explode);
        ...
        for(int i = 0; i < SLICES_WIDTH; i++){
            ...
            for(int j = 0; j < SLICES_HEIGHT; j++){
                int y = j * sliceH;
                Bitmap part = Bitmap.createBitmap(original, x, y, sliceW, sliceH); // x y
                Path tri = new Path();
                tri.moveTo(0, 0);
                for(float k = 0; k <= 1; k +=.2){
                    tri.lineTo(rnd.nextInt(sliceW), sliceH * k);
                }
                tri.lineTo(0, sliceH);
                tri.close();
    ...
    public void draw(Canvas canvas) {
        ...
        if(mStatus != STATUS_EXPLODED){
            ...
            for (int i = 0; i < fragments.size(); i++){
                ...
                backed.drawBitmap(part.bmp, 100, 100, null);
                backed.restore();
